...
const Carousel: React.FC<CarouselProps> = ({
  ...
  return (
    <div className={`relative w-full ${containerClassName}`}>
      ...
      {showLeftGradient && (
        <div
          className="absolute left-0 top-0 bottom-0 w-6 bg-gradient-to-r from-white dark:from-[#1F1F1F] to-transparent z-10"
          style={{
            opacity: 1,
            filter: 'blur(0px)',
      ...
      {showRightGradient && (
        <div
          className="absolute right-0 top-0 bottom-0 w-6 bg-gradient-to-l from-white dark:from-[#1F1F1F] to-transparent z-10"
          style={{
            opacity: 1,
            filter: 'blur(0px)',
            transition: 'opacity 0.3s ease-in-out, filter 0.3s ease-in-out',
