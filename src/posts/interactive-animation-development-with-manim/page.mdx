import { SITE_URL, AUTHOR } from '@/config';

export const metadata = {
  title: 'Interactive Animation Development with Manim',
  description: 'Discover how to set up an interactive editing environment with Manim to create sleek, professional animations for illustrating mathematical concepts.',
  image: '/images/interactive-animation-development-with-manim/interactive-animation-development-with-manim.webp',
  date: '2025-01-02',
  author: 'Patrick Prunty',
  authorUrl: 'https://patrickprunty.vercel.app/about',
  openGraph: {
    title: 'Interactive Animation Development with Manim',
    description: 'Discover how to set up an interactive editing environment with Manim to create sleek, professional animations for illustrating mathematical concepts.',
    url: '/blog/interactive-animation-development-with-manim',
    images: [
      {
        url: `${SITE_URL}/images/interactive-animation-development-with-manim/interactive-animation-development-with-manim.webp`,
        alt: 'Interactive Animation Development with Manim'
      }
    ],
    type: 'article',
    tags: [
      'mathematics',
      'programming',
      'manim',
      'content creation',
      'animation',
      'software developement',
      'python'
    ]
  },
  twitter: {
    card: 'summary_large_image',
    title: 'Interactive Animation Development with Manim',
    description: 'Discover how to set up an interactive editing environment with Manim to create sleek, professional animations for illustrating mathematical concepts.',
    image: `${SITE_URL}/images/interactive-animation-development-with-manim/interactive-animation-development-with-manim.webp`
  },
  keywords: [
    'mathematics',
    'programming',
    'manim',
    'content creation',
    'animation',
    'software developement',
    'python'
  ],
  slug: 'interactive-animation-development-with-manim',
  readingTime: 1,
  draft: true
};

Start adding your blog post content here...
